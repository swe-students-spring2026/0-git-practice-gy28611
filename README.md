# Git Practice
A simple project to practice a few git/github workflows.  Replace the contents of this file with the contents indicated in the [instructions](./instructions.md).

## Article I found interesting
I read “[How to Write a Git Commit Message](https://cbea.ms/git-commit/)” by Chris Beams. It explains a simple structure for commit messages—clear subject line, optional body, and helpful context—that makes a project’s history readable and useful.

What I found interesting is that commit messages aren’t just “notes to yourself,” they’re communication for anyone who reads the history later (teammates, future you, even automated tools). A good message makes it easier to review changes, debug regressions, and understand why decisions were made. The article also gives practical rules (like using the imperative mood and keeping the subject line short) that are easy to apply immediately.
